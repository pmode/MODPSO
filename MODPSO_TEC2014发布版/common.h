...
//	cout<<A.size()<<" clusters have been detected"<<endl;
	...
	if (SignedFlag == 0)
	{
		...
		if (!strcmp(strReal, "RealWorld\\20node.txt")) R = _20node_QR;

		if (!strcmp(strReal, "RealWorld\\2node\\test.txt")) R = test_QR;
...
/*-----remove same point in the PF----*///Pareto Front?
...
bool check_label()
{
	if (!strcmp(strlabel , "RealWorld\\new1000-6label.txt") || 
		...
		!strcmp(strlabel , "RealWorld\\2node\\real_test.txt")||
